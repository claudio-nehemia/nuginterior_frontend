...
export default function DesainFinalFormPage() {
  ...
  const fetchDetails = async () => {
    try {
      ...
      if (matchedDf) {
        ...
        // If not found, let's initialize it by posting response
        const initRes = await api.post(`/desain-finals/${orderId}/response`);
        setDesainFinal(initRes.data.data);
      }
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Gagal memuat detail data desain final');
  ...
  const handleUploadFinal = async (filesToUpload: FileList) => {
    if (!order || !filesToUpload || filesToUpload.length === 0) return;
    setIsUploading(true);

    const formData = new FormData();
    formData.append('order_id', order.id.toString());
    for (let i = 0; i < filesToUpload.length; i++) {
      formData.append('moodboard_final[]', filesToUpload[i]);
    }

    try {
      await api.post('/desain-finals/upload', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      toast.success('Berkas desain final berhasil diunggah');
      await fetchDetails();
    } catch (err: any) {
  ...
  const handleDeleteFile = async (fileId: number) => {
    ...
    try {
      await api.delete(`/desain-finals/files/${fileId}`);
  ...
  const handleReplaceFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !replacingFileId || !order) return;

    setIsUploading(true);
    try {
      // 1. Delete the old file
      await api.delete(`/desain-finals/files/${replacingFileId}`);
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-[50vh] text-gray-500 font-semibold text-xs">
        ...
        Memuat editor desain final...
      </div>
    );
  }

  if (!order || !desainFinal) {
    return (
      <div className="text-center py-12">
        <AlertCircle className="mx-auto text-red-500 mb-2" size={32} />
        <p className="text-gray-500 text-sm font-bold">Data tidak ditemukan</p>
        <Button onClick={() => navigate('/dashboard/desain-final')} className="mt-4 bg-teal-500 text-white">Kembali</Button>
      </div>
    );
  }

  const finalFiles = desainFinal.files || [];
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-2 border-b border-gray-100 pb-5">
        ...
        <div className="flex items-center gap-2.5">
          <h1 className="text-xl font-extrabold text-gray-800 tracking-tight">
            Upload & Edit Desain Final: {order.nama_project}
          </h1>
      ...
      {/* Revision notes banner if status is revision */}
      {desainFinal.status === 'revision' && finalFiles.some(f => f.status === 'revisi' && f.revisi) && (
        <Card className="border border-red-100 bg-red-50/20 rounded-2xl overflow-hidden">
          <CardContent className="p-5 flex flex-col gap-3">
            <div className="flex items-center gap-2 text-red-600">
              <AlertCircle className="shrink-0 animate-bounce" size={16} />
              <span className="text-[10px] font-black uppercase tracking-widest">Catatan Revisi dari Client / PM</span>
            </div>
            <div className="space-y-2">
              {finalFiles.filter(f => f.status === 'revisi' && f.revisi).map((f, idx) => (
                <div key={f.id} className="text-xs font-medium text-gray-700 bg-white border border-red-100/50 rounded-xl p-3 shadow-sm">
                  <span className="font-bold text-[10px] text-gray-500 block mb-1">Opsi #{idx + 1} ({f.original_name}):</span>
                  <p className="whitespace-pre-wrap">{f.revisi}</p>
                </div>
              ))}
            </div>
          </CardContent>
          ...
          <Card className="border-0 shadow-sm rounded-2xl bg-white overflow-hidden">
            <CardHeader className="border-b border-gray-50">
              <CardTitle className="text-sm font-extrabold text-gray-800">
                Kelola Berkas Desain Final
              </CardTitle>
              <CardDescription className="text-[10px] text-gray-500">
                Mendukung PDF, PNG, JPG, dan format lainnya. Berkas ini akan menjadi visual blueprint akhir.
              </CardDescription>
            </CardHeader>
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.files && e.target.files.length > 0) {
                      handleUploadFinal(e.target.files);
                    }
                  }}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-extrabold text-gray-800">
                    Berkas Terunggah ({finalFiles.length})
                  </h3>
                  <span className="text-[9px] font-bold text-gray-400">
                    Total: {finalFiles.length} file
                  </span>
                </div>
                ...
                {finalFiles.length === 0 ? (
                  <div className="text-center py-8 text-gray-400 text-xs italic bg-gray-50/30 rounded-xl border border-dashed border-gray-100">
                    Belum ada file desain final yang diunggah
                  </div>
                ) : (
        ...
        <div className="space-y-6">
          <Card className="border-0 shadow-sm rounded-2xl bg-white overflow-hidden">
            ...
            <CardContent className="p-5 text-xs text-gray-500 space-y-3 leading-relaxed">
              <p>
                Silakan upload opsi desain final visual yang sudah dimatangkan dan disepakati. Client atau Project Manager akan meninjau opsi-opsi visual ini.
              </p>
              ...
              <p>
                Setelah salah satu opsi disetujui, alur proyek akan langsung berlanjut ke tahap penginputan RAB (Rencana Anggaran Biaya) detail untuk pembuatan kontrak kerja.
              </p>
            </CardContent>
          </Card>
