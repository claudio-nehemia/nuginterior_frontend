...
interface InputRoomBahanBaku {
	id: number;
	bahan_baku_id: number;
	nama_bahan: string;
}

interface InputRoomFinishing {
	id: number;
	item_id: number;
	nama_item: string;
	notes: string;
...
interface InputRoom {
	...
	lebar: number;
	tinggi: number;
	bahan_bakus: InputRoomBahanBaku[];
	finishing_dalams: InputRoomFinishing[];
	finishing_luars: InputRoomFinishing[];
	aksesoris: InputRoomAksesoris[];
}

interface InputItemDetail {
	id: number;
...
export default function RabCreatePage() {
	const { inputItemId } = useParams();
	const navigate = useNavigate();
	const location = useLocation();
	const orderId = location.state?.orderId;

	const [inputItem, setInputItem] = useState<InputItemDetail | null>(null);
	const [products, setProducts] = useState<Product[]>([]);
	const [items, setItems] = useState<Item[]>([]);
	const [isLoading, setIsLoading] = useState(true);

	// General RAB configuration states
	const [markupGeneral, setMarkupGeneral] = useState<string>('10'); // Default 10%
	const [roomsState, setRoomsState] = useState<RoomRABState[]>([]);

	const loadData = async () => {
		setIsLoading(true);
		try {
			const [iiRes, prodRes, itemsRes] = await Promise.all([
				api.get(`/input-items/${inputItemId}`),
				api.get('/produk'),
				api.get('/items')
			]);

			const iiData: InputItemDetail = iiRes.data.data;
			setInputItem(iiData);
			setProducts(prodRes.data.data || []);
			setItems(itemsRes.data.data || []);

			// Initialize rooms state based on input item structure
			const initialRooms = iiData.rooms.map(room => ({
				...
				lebar: room.lebar,
				tinggi: room.tinggi,
				markup: '10', // Default markup room
				bahan_bakus: room.bahan_bakus.map(bb => ({ bahan_baku_id: bb.bahan_baku_id, markup: '10' })),
				finishing_dalams: room.finishing_dalams.map(f => ({ item_id: f.item_id, markup: '10' })),
				finishing_luars: room.finishing_luars.map(f => ({ item_id: f.item_id, markup: '10' })),
				aksesoris: room.aksesoris.map(a => ({
					item_id: a.item_id,
					qty: a.qty,
					markup: '10' // Default markup aksesoris
				}))
			}));
			setRoomsState(initialRooms);

		} catch (err: any) {
			toast.error('Gagal memuat rincian data: ' + (err.response?.data?.message || err.message));
	...
	// Helper to lookup product material prices from pivot
	...
	// Helper to lookup items prices (finishing/accessories)
	...
	const calculateRoomCosts = (room: RoomRABState) => {
		...
		// Room Harga Satuan = Harga Dasar * (Panjang * Lebar * Tinggi) * Qty (Produk)
		const volume = room.panjang * room.lebar * room.tinggi;
		const hargaSatuan = hargaDasar * volume * room.qty;
		// Room Harga Total = Harga Satuan + Total Aksesoris
	...
	const handleSaveRAB = async () => {
		try {
			const payload = {
				input_item_id: parseInt(inputItemId!),
				order_id: inputItem?.order_id || orderId,
			...
			await api.post('/rab', payload);
			toast.success('Dokumen RAB berhasil dibuat');
			navigate('/dashboard/rab');
		} catch (err: any) {
			toast.error('Gagal menyimpan RAB: ' + (err.response?.data?.message || err.message));
		}
	};

	if (isLoading || !inputItem) {
		return (
		...
		<div className="space-y-5 animate-in fade-in duration-500">
			...
			<div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
				<div className="flex items-center gap-3">
					...
					<div>
						<h1 className="text-base font-extrabold text-gray-800 tracking-tight">Buat RAB Baru</h1>
						<p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">
							Project: {inputItem.order?.nama_project} &bull; Client: {inputItem.order?.nama_customer}
						</p>
					</div>
				</div>
				<Button
				...
				>
					<Save size={14} className="mr-1.5" /> Simpan RAB (Draft)
				</Button>
			</div>
			...
			<div className="space-y-6">
				<h2 className="text-xs font-black text-gray-400 uppercase tracking-widest">Detail Pekerjaan per Ruangan</h2>
										...
										<Table>
											...
											<TableBody>
												{room.aksesoris.map((aks, aIdx) => {
													...
													const aksMarkupFloat = parseFloat(aks.markup) || 0;
													const aksTotal = price * (1 + aksMarkupFloat / 100) * aks.qty;
